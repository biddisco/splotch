...
timer.start();
	...
	cu_get_fbuf(fragBuf, posInFragBuf);//posInFragBuf comes from filtering
timer.stop();
time =timer.getTime();
		cout << endl << "Render to fragment buffer cost time:" << time << "s" <<endl;

	//gold compare the fragment buffers
	//GoldCompareFBuf(fragBufWrittenByHost, fragBuf, posInFragBuf);	

timer.reset();
timer.start();
	  //here we test combination of host fragment
	  cu_fragment_AeqE	*bufWrite;//just for test
	  bufWrite=fragBuf;
	  //bufWrite =fragBufWrittenByHost;
	  for (int pPos=0, fPos=0; pPos<pFiltered && fPos<posInFragBuf/*posFragBufH*/; pPos++)
	  {
...
timer.stop();
time =timer.getTime();
	cout << endl << "Combining fragment buffer cost time:" << time << "s" <<endl;
