...
void GoldComparePData  (vector < particle_sim > particle_data, cu_particle_sim * d_particle_data);
void GoldCompareSData  (vector < particle_splotch > host_data, cu_particle_splotch * device_data);
...
// ----------------------------------------------
  ...
  for(int itype = 0; itype < ptypes; itype++)
	...
	
	{
	  ...
	  planck_assert(infile,
			 "could not open palette file  <" + params.find < string >
			 ("palette" + dataToString(itype)) + ">");
	  string dummy;
	  int nColours;
	  ...
	  for(int i = 1; i < nColours; i++)
	    
	    {
	      ...
	      amap[itype].
		Add_Entry(new
			  LINEAR_CMAP_ENTRY((i - 1) * step, i * step,
...
// and so on ...
  ...
  COLOUR c1, c2, c3  c1 = COLOUR(1, 0, 0);	// red
...
// -------------------------------------
  ...
  while(getline(inp, line))
    ...
    {
      sscanf(line.c_str(), "%lf %lf %lf %lf %lf %lf %lf %lf %lf", &campos.x, &campos.y, &campos.z,
	      &lookat.x, &lookat.y, &lookat.z, &sky.x, &sky.y, &sky.z);
      if(master)
...
//                  bin_reader_block(particle_data, &maxr, &minr, mpiMgr.rank(), mpiMgr.num_ranks());
	...
	case 2:
...
#ifdef INTERPOLATE
	    cout << "File1: " << snr1_this << " , File2: " << snr2_this << " , interpol fac: " << frac <<
	    endl;
	  ...
	  if(master)
	    cout << "Interpolating between " << particle_data1.size() << " and " << particle_data2.
	      size() << " particles ..." << endl;
...
*/ 
    ...
    timer.reset();
  timer.start();
  double c[3] = { campos.x, campos.y, campos.z },  l[3] =
  {
...
//                cout<< endl << "CAUSION: SORTING NEEDED!"<<endl;
  ...
  for(int i = 0; i < size_inout_buffer; i++)
	...
	r = inout_buffer[i][2], g = inout_buffer[i][3],  b = inout_buffer[i][4];
      if(type == 0)
	continue;
      cu_color clr;
...
//                      amapD, size, amapDTypeStartPos, ptypes);
  ...
  for(int i = 0; i < size; i++)
	...
	
	{
	  ...
	  posInFragBuf += (cu_ps[i].maxx - cu_ps[i].minx) * (cu_ps[i].maxy - cu_ps[i].miny);
...
void  GoldCompareFBuf(cu_fragment_AeqE * goldBuf, cu_fragment_AeqE * buf, int n) 
{
  for(int i = 0; i < n; i++)
    ...
    {
      if(abs(goldBuf[i].deltaR - buf[i].deltaR) < 1.0e-5 && abs(goldBuf[i].deltaG - buf[i].deltaG) < 1.0e-5
...
void GoldComparePData  (vector < particle_sim > particle_data, cu_particle_sim * d_particle_data) 
{
  for(int i = 0; i < particle_data.size(); i++)
    ...
    {
      if(particle_data[i].x == d_particle_data[i].x && particle_data[i].y == d_particle_data[i].y
	  && particle_data[i].z == d_particle_data[i].z && particle_data[i].r == d_particle_data[i].r
	  && particle_data[i].ro == d_particle_data[i].ro && particle_data[i].I == d_particle_data[i].I
	  && particle_data[i].C1 == d_particle_data[i].C1 && particle_data[i].C2 == d_particle_data[i].C2
...
void GoldCompareSData  (vector < particle_splotch > host_data, cu_particle_splotch * device_data) 
{
  for(int i = 0; i < host_data.size(); i++)
    ...
    {
      if(host_data[i].x == device_data[i].x && host_data[i].y == device_data[i].y
	  && host_data[i].r == device_data[i].r && host_data[i].ro == device_data[i].ro )
	continue;
