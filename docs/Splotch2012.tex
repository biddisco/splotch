...
Table XXX summarize the main characteristics of the datasets adopted in the 
tests. The 100M dataset is made of 100 million particles, distributed in 
...
demanding from the memory point of view. The dataset, by itself, requires 2 GB 
of memory, so it is at the upper limit of many GPU, but also CPU, memories.
The 100M test can be run only on GPU cards with memory of 4 or 6 GBs.
The 10M dataset is a random 
selection of $10\%$ of the 100M particles, created for performing tests 
on any available graphic card and CPU The memory requirements, in fact, 
are not so challenging as for the 100M case (ten times smaller).
...
The GALAXY dataset is characterized 
