...
Algorithms for data mining and analysis are often computationally demanding and in practice unusable on big datasets
due to time constraints and power limitations. Exploration and discovery through visualization can then be an outstanding aid,
...
supporting a number of popular formats for astrophysics. Once datasets are converted into the Splotch internal format as they are 
loaded, followed by preprocessing and rasterisation, and finally rendering. Preprocessing can perform ranging, normalization, 
...
The new implementation presented exploits the MIC architecture. The idea behind MIC is obtaining a massive level of parallelism for 
increasing throughput performance in power restricted cluster environments. To this end Intel's flagship MIC product, the Xeon Phi,
contains roughly 60 cores on a single chip, dependent on the model. The Xeon Phi acts as a coprocessor for a standard Intel Xeon 
processor. Programs can be executed natively by logging into the device itself, which hosts a Linux micro-OS, or by using the 
device through one or more MPI processes amongst further processes running on the Xeon host. Alternatively users can offload 
data and portions of code to the coprocessor via a series of pragma based extensions available in C++ or FORTRAN.

A brief description of the Xeon Phi is provided for our purposes, for a detailed technical description the reader is referred to the
Xeon Phi whitepaper \cite{xeonphi}. Each core has access to a 512 KB private fully coherent L2 cache and memory controllers
...
The first stage of processing is rasterization: a highly parallel 3D transform and colorize performed on a per-particle
basis. Transform parameters are precomputed, as they are the same for each particle, and computation is distributed amongst
threads via an OpenMP parallel for loop.  We use four OpenMP threads per available core to match the four available hardware threads, 
equating to roughly 240 threads. Further tuning enabled this loop to be auto-vectorized by the Intel compiler providing a large 
...
containing the indices of all particles affecting the tile.
...
memory roughly 2-2.5x faster having set this environment variable to 64K. The inability to asynchronously allocate offload 
memory means that overheads incurred allocating these buffers cannot be mitigated by overlapping allocation with host activity.
...
minimised by placing an empty offload clause with empty OpenMP parallel section near to the beginning of the program, in order 
...
For performance comparison, both the host and device code use a tile size parameter of 100 pixels.

In the tests measuring the device performance, we run 8 MPI processes on the host all offloading to the same device, 
as discussed in sect. \ref{sect:memusage} and set thread groups of 15 threads each, which turned to to be the optimal distribution for this hardware
...
\end{minipage}
\caption{\emph{Left:} Per-Frame time for full process: host OpenMP 1-16 cores vs single and dual Xeon Phi devices.
\emph{Right:} Per-Frame time for rasterization phase: host OpenMP 1-16 cores vs single and dual Xeon Phi devices.}
\label{fig:timings}
\end{figure}
...
shows use of a single device provides comparable results to 4 cores on the host, while two devices outperform 16 cores by 20\%, due to the 
...
device outperforming 16 cores roughly 2.5x, with roughly 3x improvement provided by using dual devices. The differing performances 
betweeen fig. \ref{fig:timings} (\emph{left}) and fig. \ref{fig:timings} (\emph{right}) are due to the rendering phase, which does not 
gain as significant a performance boost as the rasterization. 
...
testing to be done in order to perfect the double buffered data transfer and processing approach to effectively visualize much larger 
...
\bibitem{xeonphi} Xeon Phi whitepaper reference

\bibitem{mic} MIC architecture whitepaper reference 

\bibitem{mem_alloc} High cost of dynamic memory allocations reference

\bibitem{env_var_buf} Benefit of 2MB buffers reference 

\bibitem{imci} Manycore instruction set reference

\bibitem{vectorguide} Vectorization guide reference

\bibitem{gadget} http://www.mpa-garching.mpg.de/gadget/

\bibitem{knightlanding} ISC2013 knights landing release reference
