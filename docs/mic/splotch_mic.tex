...
The Splotch algorithm can be broken down into a series of phases illustrated by the execution model shown in fig. \ref{fig:exmodel}.
...
The rendering stage begins with splitting the number of available OpenMP threads into groups, the number of which is dependant on 
...
consists in processing the data concurrently by both the multiple host cores, coordinated through the MPI
implementation, and by the device.
Data to be processed by the MIC can be off-loaded asynchronouly by the various host's cores.
While data from one core are copied to the device, the same core can continue its 
