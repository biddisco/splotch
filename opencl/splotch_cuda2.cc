...
void cuda_rendering(int mydevID, int nDev, arr2<COLOUR> &pic)
  {
cout<<mydevID<<"geia"<<nDev<<"geia"<<pic.size1()<<"geia"<<pic.size2()<<"geia"<<endl;
...
cu_fragment_AneqE *fragBuf2;
  ...
  while (End_cu_ps < nParticle)
  {
   ...
   tInfo->times.stop("grender");
    ...
    combine_chunk2(0, pFiltered-1, cu_ps_filtered, fragBuf2, a_eq_e, grayabsorb, *(tInfo->pPic));
		...
		cu_fragment_AeqE *fragBuf, bool a_eq_e, float64 grayabsorb, arr2<COLOUR> &pPic)
{
      ...
      for (int pPos=StartP, fPos=0; pPos<EndP; pPos++)
      {
        for (int x =cu_ps_filtered[pPos].minx; x <cu_ps_filtered[pPos].maxx; x++)
        {
          ...
          {
            pPic[x][y].r += fragBuf[fPos].aR;
            pPic[x][y].g += fragBuf[fPos].aG;
            pPic[x][y].b += fragBuf[fPos].aB;
            fPos ++;
          }
        }
      }
		...
		cu_fragment_AneqE *fragBuf, bool a_eq_e, float64 grayabsorb, arr2<COLOUR> &pPic)
{
      ...
      for (int pPos=StartP, fPos=0; pPos<EndP; pPos++)
      {
        for (int x =cu_ps_filtered[pPos].minx; x <cu_ps_filtered[pPos].maxx; x++)
        {
          ...
          {
            pPic[x][y].r += fragBuf[fPos].aR *
				   (pPic[x][y].r - fragBuf[fPos].qR);
            pPic[x][y].g += fragBuf[fPos].aG *
				   (pPic[x][y].g - fragBuf[fPos].qG);
            pPic[x][y].b += fragBuf[fPos].aB *
                                   (pPic[x][y].b - fragBuf[fPos].qB);
            fPos ++;
          }
        }
      }
